...
def evaluate(the_input, x_set, batchcount=1, accuracy_ontrain=0.9988):
   ...
   def single_eval(x):
     ...
     thenet.blobs['data'].data[:] = the_input.value[:]
...
for itr in range(niter):
   ...
   if itr < 2000 and itr%10000 == 0:#=False except itr==0??
   ...
   if itr%1000==0 and len(tmp_ind)>1 and itr < prune_stop_iter:# run at window @3
      ...
      if es_method == 'ncs':
         __C = edict()
         __C.parameters = {'reset_xl_to_pop':False,'init_value':tmp_crates, 
                           'stepsize':ncs_stepsize, 'bounds':[0.0, 10.], 'ftarget':0, 'tmax':1600, 'popsize':10, 'best_k':1}
